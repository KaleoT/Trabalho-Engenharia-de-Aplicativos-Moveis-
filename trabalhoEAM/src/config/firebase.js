...
// UPDATE
await
firebase.firestore().collection('clientes').doc(clienteAtual.id).update({
...
try {
await
firebase.firestore().collection('clientes').doc(cliente.id).delete();
carregarClientes();
...
{/* Header */}
<Appbar.Header>
<Appbar.Content title="Clientes" subtitle={`${clientes.length} clientes
cadastrados`} />
...
<Paragraph>Clique no botão + para adicionar um novo
cliente</Paragraph>
<Button
mode="contained"
onPress={novoCliente}
...
onChangeText={(text) => setClienteAtual({...clienteAtual, codigo:
text})}
mode="outlined"
style={styles.input}
...
onChangeText={(text) => setClienteAtual({...clienteAtual, nome:
text})}
mode="outlined"
style={styles.input}
...
onChangeText={(text) => setClienteAtual({...clienteAtual, foto:
text})}
mode="outlined"
style={styles.input}
